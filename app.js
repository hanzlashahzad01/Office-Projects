...
// NOTE: Dynamic path drawing removed – the new roadmap layout uses
// static SVG connector paths embedded directly in the HTML template.
...
function openRoadmap(event) {
    ...
    // Attach click listener for Watch Lecture button → Dashboard.html
    ...
    // Attach click listener for Revise Slides button → slides.html
    const reviseSlidesBtn = roadmapContent.querySelector(".btn-revise-slides");
    if (reviseSlidesBtn) {
        reviseSlidesBtn.addEventListener("click", () => {
...
function showRoadmapFromLecture() {
    ...
    // Static SVG paths are used – no redraw needed.
}
...
// No dynamic path drawing needed – roadmap uses static embedded SVG connectors.
